...
export type AuthModalMode = 'signin' | 'signup' | 'recovery';
...
interface AppContextType {
  ...
  isAuthModalOpen: boolean; authModalMode: AuthModalMode; openAuthModal: (mode?: AuthModalMode) => void; closeAuthModal: () => void;
  ...
  signInWithPassword: (email: string, password: string) => Promise<void>; signUpWithPassword: (email: string, password: string, name: string, phone: string, role: Exclude<UserRole, 'admin'>) => Promise<{ confirmationRequired: boolean }>; requestPasswordReset: (email: string) => Promise<void>; updatePassword: (password: string) => Promise<void>; logout: () => Promise<void>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [isSiteVisitOpen, setIsSiteVisitOpen] = useState(false); const [isInquiryOpen, setIsInquiryOpen] = useState(false); const [isUnitConverterOpen, setIsUnitConverterOpen] = useState(false); const [isVerificationModalOpen, setIsVerificationModalOpen] = useState(false); const [isAuthModalOpen, setIsAuthModalOpen] = useState(false); const [authModalMode, setAuthModalMode] = useState<AuthModalMode>('signin'); const [selectedPropertyForModal, setSelectedPropertyForModal] = useState<Property>();
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => { if (event === 'PASSWORD_RECOVERY') { setAuthModalMode('recovery'); setIsAuthModalOpen(true); } void refreshData().catch(console.error); });
  ...
  const signInWithPassword = async (email: string, password: string) => { if (!supabase) throw new Error('Authentication is not configured.'); const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error) throw error; setIsAuthModalOpen(false); showToast('Signed in successfully.', 'success'); };
  const signUpWithPassword = async (email: string, password: string, name: string, phone: string, role: Exclude<UserRole, 'admin'>) => { if (!supabase) throw new Error('Authentication is not configured.'); const { data, error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: window.location.origin, data: { full_name: name, phone, requested_role: role } } }); if (error) throw error; const confirmationRequired = !data.session; if (!confirmationRequired) { setIsAuthModalOpen(false); showToast('Account created successfully.', 'success'); } return { confirmationRequired }; };
  const requestPasswordReset = async (email: string) => { if (!supabase) throw new Error('Authentication is not configured.'); const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: window.location.origin }); if (error) throw error; showToast('Check your email for the password reset link.', 'success'); };
  const updatePassword = async (password: string) => { if (!supabase) throw new Error('Authentication is not configured.'); const { error } = await supabase.auth.updateUser({ password }); if (error) throw error; setAuthModalMode('signin'); setIsAuthModalOpen(false); showToast('Password updated. You are signed in.', 'success'); };
